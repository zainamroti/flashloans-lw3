...
describe("Deploy a Flash Loan", function () {
    it("Should take a flash loan and be able to return it", async function () {
        ...
        const BALANCE_AMOUNT_DAI = ethers.utils.parseEther("100");
        ...
        const tx = await _flashLoanExample.createFlashLoan(DAI, 100); // Borrow 1000 DAI in a Flash Loan with no upfront collateral
        await tx.wait();
        ...
        console.log(`RemBal: ${remainingBalance}`)
        expect(remainingBalance.lte(BALANCE_AMOUNT_DAI)).to.be.true; // We must have less than 2000 DAI now, since the premium was paid from our contract's balance
    });
});
